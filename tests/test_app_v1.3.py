...
import pytest
import json
from app_v1_3 import app as flask_app, calculate_calories, calculate_bmi, calculate_bmr
...
def test_health_check_v1_3(client):
...
def test_calorie_calculation():
    ...
    calories = calculate_calories(70, 6.0, 30)
    assert calories > 0
...
def test_bmr_calculation():
    ...
    bmr_male = calculate_bmr(70, 175, 30, 'M')
    bmr_female = calculate_bmr(70, 175, 30, 'F')
    assert bmr_male > 0
    assert bmr_female > 0
...
def test_workout_with_calories(client):
